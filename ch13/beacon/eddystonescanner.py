            ...
            while True:
                if srv_ident != True or dat_ident != True:
                    ...
                    if typeval == 0x02 or typeval == 0x03: #list of 16bit Service UUIDs
                        ...
                        elif srvtype == 0x10: # Eddystone-URL
                        ...
                        elif srvtype == 0x20: # Eddystone-TLM
                            if length != 0x11:
                                print "Not valid eddystone-TLM frame!"
                                break
                            vbatt=int(data[18:23].replace(' ',''),16)
                            temp=int(data[24:29].replace(' ',''),16)
                            advcnt=int(data[30:41].replace(' ',''),16)
                            seccnt=int(data[42:53].replace(' ',''),16)
                            print "VBATT: %fV TEMP: %f ADVCNT: %d SECCNT: %d" % (vbatt/1000, temp/256., advcnt, seccnt)
                        dat_ident = True
